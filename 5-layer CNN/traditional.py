...
# This file contians code directly taken from the following repositories.
# PyCrCNN Source: https://github.com/AlexMV12/PyCrCNN (for implementing the neural network parts)
# Pyfhel Source: https://github.com/ibarrond/Pyfhel (for implementing the FHE parts)
...
def indi_sum(t):
    if isinstance(t[0], np.ndarray):
        raise TypeError
    return np.sum([t[i] for i in range(0, len(t))])
    
def indi_sum_wrapper(t):
    try:
        return indi_sum(t)
    except TypeError:
        return np.sum([indi_sum_wrapper(sub) for sub in t])

def square_sum(HE, t, t_mean):
    if isinstance(t[0], np.ndarray):
        raise TypeError
    sum = HE.power(t[0]- t_mean, 2)
    for i in range(1, len(t)):
        sum += HE.power(t[i]-t_mean, 2)
    return sum

def square_sum_wrapper(HE, t, t_mean):
    try:
        return square_sum(HE, t, t_mean)
    except TypeError:
        return np.sum([square_sum_wrapper(HE, sub, t_mean) for sub in t])
...
class AveragePoolLayer:
    ...
    def __call__(self, t):
        ...
        print("avg time: " + str(time.time()-start))
        return result
...
if train:
    ...
    torch.save(LeNet1_Approx, "model.pt")
...
lenet_1_approx = torch.load("model.pt", map_location="cpu")
