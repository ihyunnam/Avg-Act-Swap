...
# This file contains code directly taken from the following repositories.
# PyCrCNN: https://github.com/AlexMV12/PyCrCNN (for implementing the neural network parts)
# Pyfhel: https://github.com/ibarrond/Pyfhel (for implementing the FHE parts)
...
class CKKSPyfhel(HE):
    def __init__(self, m=2**15, scale=2**30, qi=[31, 30, 30, 30, 30, 30, 30, 30, 30, 30,30, 30, 30, 30, 30, 30,31]):
        self.he = Pyfhel()
...
class LinearLayer:
    ...
    def __call__(self, t):
        ...
        print("linear layer: " + str(time.time()-start))
        return result
...
# Tanh acivation layer, linearly approximated
class TanhLayer:
...
def _avg(HE, image, kernel_size, stride):
    ...
    if (image.shape[1] == 10):
        denom = x_k * y_k
        denom = denom - 0.333*denom**3 + 0.133*denom**5
...
class Sequential:
    ...
    def __init__(self, HE_Server, HE_Client, model):
        ...
        def avg_pool_layer(layer):
            ...
            return AveragePoolLayer(self.HE, kernel_size, stride, padding,)
...
class Tanh(nn.Module):
    ...
    def forward(self, t):
        return t - torch.pow(t,3)/3 + torch.pow(t,5)*2/15

transform = transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32)), transforms.Normalize(mean=[0.1307], std=[0.3081])])
...
if train:
    ...
    torch.save(LeNet1_Approx, "lenet5.pt")

# CKKS cryptographic params
n_mults = 15
m = 2**15
scale_power = 30
...
lenet_1_approx = torch.load("lenet5.pt", map_location="cpu")
...
for i in range(100):
    ...
    print("avg time so far: " + str(total_time/(i+1)))
